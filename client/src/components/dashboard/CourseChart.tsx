...
export function CourseChart({ data }: CourseChartProps) {
  ...
  const handleCourseClick = (courseName: string) => {
    setLocation(`/students?courseInterested=${encodeURIComponent(courseName)}`);
  };
    ...
    return (
      <div className="space-y-3 py-2">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex items-center justify-between border-b pb-2 last:border-b-0 cursor-pointer hover:bg-accent/50 rounded-md px-2 py-1 transition-colors"
