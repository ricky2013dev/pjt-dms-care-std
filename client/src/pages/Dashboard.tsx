          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
              <MetricCard
                ...
                icon={Users}
                onClick={() => setLocation('/students?view=all')}
              />
              <MetricCard
                title="Active/Enrolled"
                value={data.metrics.active}
                icon={UserCheck}
                onClick={() => setLocation('/students?status=active,enrolled')}
              />
              <MetricCard
                title="Graduated"
                value={data.metrics.graduated}
                icon={GraduationCap}
                onClick={() => setLocation('/students?status=graduated')}
              />
              <MetricCard
                title="New This Month"
                value={data.metrics.newThisMonth}
                icon={UserPlus}
                onClick={() => setLocation(`/students?registrationDateFrom=${getFirstDayOfMonth()}`)}
              />
            </div>
