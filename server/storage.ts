...
import { db } from "./db";
import { eq, like, ilike, and, gte, lte, desc, sql, inArray, or } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllStudents(filters?: StudentFilters): Promise<{ students: Student[]; total: number }> {
    ...
    if (filters?.courseInterested) {
      // Support both single course and comma-separated multiple courses (OR logic)
      const courses = filters.courseInterested.split(',').map(c => c.trim()).filter(Boolean);
      if (courses.length > 0) {
        // Use inArray for OR logic: WHERE courseInterested IN ('CNA', 'CCMA')
        conditions.push(inArray(students.courseInterested, courses as [string, ...string[]]));
      }
    }
    if (filters?.location) {
    ...
    if (filters?.status) {
      // Support both single status and comma-separated multiple statuses (OR logic)
      const statuses = filters.status.split(',').map(s => s.trim()).filter(Boolean);
      console.log('Server received status filter:', filters.status, 'parsed as:', statuses);
      if (statuses.length > 0) {
        // Use inArray for OR logic: WHERE status IN ('active', 'enrolled')
        conditions.push(inArray(students.status, statuses as [string, ...string[]]));
        console.log('Created inArray condition for statuses:', statuses);
      }
    }
